...
function prepare_user_sql_query($is_count)
{
    ...
    if ($is_count) {
        ...
        if (api_is_western_name_order()) {
            $sql .= "concat(u.firstname,' ',coalesce((select value from extra_field_values where field_id=23 and item_id=u.id),'')) AS col3, concat(u.lastname,' ',coalesce((select value from extra_field_values where field_id=24 and item_id=u.id),'')) AS col4, ";
        } else {
            $sql .= "concat(u.lastname,' ',coalesce((select value from extra_field_values where field_id=24 and item_id=u.id),'')) AS col3, concat(u.firstname,' ',coalesce((select value from extra_field_values where field_id=23 and item_id=u.id),'')) AS col4, ";
        }

        $sql .= " u.username AS col5,
                    FNombreCursoAlumno(u.user_id) AS col6,
                    u.status AS col7,
                    u.active AS col8,
                    u.id AS col9,
    ...
    if (api_is_platform_admin()) {
        if (!$user_is_anonymous) {
            $result .= '<a href="user_information.php?user_id='.$user_id.'">'.Display::return_icon('synthese_view.gif', get_lang('Info')).'</a>&nbsp;&nbsp;';
						$result .= '<a href="http://chamilo.cloudapp.net/main/observador/index.php?cidReq=0&id_session=0&gidReq=0&gradebook=0&origin=&user_id='.$user_id.'">'.Display::return_icon('observador.png', get_lang('Observador')).'</a>&nbsp;&nbsp;';
        } else {
...
$table->set_header(6, get_lang('Curso'));
